...
//==============================================================================================
     ...
     */
    fun nextIrisPosition(
        eyePosition: PointF?,
        eyeRadius: Float,
        irisRadius: Float
    ): PointF? { // Correct the current eye position and size based on recent motion of the face within the
...
// velocity changes below are pro-rated based on this.
        ...
        if (!isStopped()) { // Only apply gravity when the iris is not stopped at the bottom of the eye.
...
// that the returned result is at a valid position within the eye.
    ...
    private fun applyFriction(
        ...
        velocity = if (isZero(velocity)) {
            0.0f
        } else if (velocity > 0) {
            Math.max(0.0f, velocity - FRICTION * simulationRate)
        } else {
            Math.min(0.0f, velocity + FRICTION * simulationRate)
        }
        return velocity
    }
    ...
    private fun makeIrisInBounds(simulationRate: Float) {
        ...
        val distance = Math.sqrt(
            Math.pow(
                irisOffsetX.toDouble(),
                2.0
            ) + Math.pow(irisOffsetY.toDouble(), 2.0)
        ).toFloat()
        if (distance <= maxDistance) { // The iris is in bounds, so no correction is necessary.
...
// the eye.
    ...
    private fun applyBounce(
        velocity: Float,
        distOutOfBounds: Float,
        simulationRate: Float
    ): Float {
        var velocity = velocity
        if (isZero(distOutOfBounds)) { // No bounce needed, since we are still in bounds along this dimension.
            return velocity
        }
...
// this quick movement, based upon how much it was out of bounds.
        val bounce =
            BOUNCE_MULTIPLIER * Math.abs(distOutOfBounds / mIrisRadius)
        if (velocity > 0) {
